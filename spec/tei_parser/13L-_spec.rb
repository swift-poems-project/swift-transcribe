...
describe 'TeiParser' do
  ...
  Dir.glob("#{@nb_store_path}/13L-/*").select {|path| not /tocheck/.match(path) and not /PUMP/.match(path) and not /tochk/.match(path) and not /TOCHECK/.match(path) }.each do |file_path|
